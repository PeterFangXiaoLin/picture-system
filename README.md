...
空间邀请与审批结果通过 `user_notification` 表持久化。每条记录只对应一个接收用户，主要字段如下：

- `userId`：通知接收人
- `type`：通知类型，包括空间邀请、邀请已接受、邀请已拒绝
- `relatedId`：关联的 `space_user.id`
- `spaceId`：关联空间
- `actorUserId`：触发通知的用户
- `isRead`：阅读状态，`0-未读 / 1-已读`
- `actionStatus`：邀请业务状态，`0-待确认 / 1-已接受 / 2-已拒绝`

阅读状态和邀请业务状态相互独立。一键全部已读只会清除未读角标，不会结束待确认邀请；受邀用户仍然可以继续选择接受或拒绝。

通知流程：

1. 空间管理员发送邀请后，系统为受邀用户创建一条待确认通知。
2. 受邀用户可以在页面右上角的通知中心接受或拒绝邀请。
3. 邀请处理成功后，受邀用户的邀请通知自动标记为已读并更新业务状态。
4. 空间内所有已接受邀请的管理员都会收到接受或拒绝结果通知。
5. 邀请创建、邀请状态更新和结果通知写入使用数据库事务，任一步骤失败都会回滚。

成员管理页面按照邀请状态分为“正式成员”“待确认”“已拒绝”三个区域。只有 `inviteStatus = 1` 的记录属于正式空间成员并拥有对应角色权限，已拒绝的用户可以由管理员重新邀请。

通知接口：

- `GET /notification/list/my`：查询当前用户最近 100 条通知
- `GET /notification/unread/count`：查询当前用户未读通知数量
- `POST /notification/read`：将指定通知标记为已读
- `POST /notification/read/all`：一键将全部通知标记为已读

前端每 30 秒刷新一次未读数量。接受邀请后会主动刷新侧边栏的团队空间列表。

已有数据库升级时，需要先执行：

```sql
source picture-system-backend/sql/create_user_notification.sql;
```

全量初始化数据库时，`picture-system-backend/sql/create_table.sql` 已包含通知表结构。
...
引入 ShardingSphere 实现动态分表

因为逻辑表是 通过 picture_{spaceId}，由于 id很长没有办法直接写到配置文件的 actual-data-nodes 中，所以需要自定义 动态分表算法，springboot 启动时，修改配置加到 actual-data-nodes 中。
...
增加空间类型枚举类，角色类型枚举，空间权限常量

创建页面，route 增加一个 type 用于区分创建的类型
