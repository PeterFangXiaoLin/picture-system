package com.my.picturesystembackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "picture_system." + tableName) // 确保前缀合法
                .collect(Collectors.joining(","));
        log.info("动态分表 actual-data-nodes 配置: {}", newActualDataNodes);

        ContextManager contextManager = getContextManager();
        String databaseName = contextManager.getMetaDataContexts()
                .getMetaData()
                .getDatabases()
                .keySet()
                .stream()
                .filter(name -> contextManager.getMetaDataContexts()
                        .getMetaData()
                        .getDatabases()
                        .get(name)
                        .getRuleMetaData()
                        ...
                        .isPresent())
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("未找到配置分片规则的 ShardingSphere 逻辑数据库"));
